...
def makePaymentDataset(pathToModels, option):
    ...
    stpwds.add('aug')
    #print(stpwds)
    unigram_freq = pd.read_csv(pathToModels + '/' + "Payment/Hundred_unigram_freq.csv")
    unigram_freq = unigram_freq.iloc[:,1]
    for words in unigram_freq:
        stpwds.add(words.lower())

    fobject = open(pathToModels + '/' + "Payment/PaymentTopWords.csv", "r")   #!Don't Change this
    ...
    for directory in dirList:
        ...
        if(os.path.isdir(selectedFolder + "/" + directory) and (selectedFolder + "/" + directory != "__pycache__")):
            ...
            fobject.close()
            print("*******************************************")
    model = SourceFileLoader("ModelModule", pathToModels + '/' + "Payment/Payment_Model_run.py").load_module()
