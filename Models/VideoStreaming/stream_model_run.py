...
def tknz(pathToModels,data_st):
    ...
    data_m = pd.read_csv(pathToModels+'/VideoStreaming/stream_t.csv', encoding = 'utf-8')
    s = data_m['text_final']
    ...
    for index,entry in enumerate(data_s['Text']):
        ...
        s.loc[len(s)] = str(proc_arr)

    print(s[len(s)-1])

    Tf = TfidfVectorizer(max_features=5000)
    Tf.fit(data_m['text_final'][0:len(s)-1])
    Train_X_tf = Tf.transform(data_m['text_final'][0:len(s)-1])
    Test_X_tf = Tf.transform([s[len(s)-1]])
    return model(Train_X_tf, Test_X_tf, data_m)
    

def cral(pathToModels,resp):
    text = resp
